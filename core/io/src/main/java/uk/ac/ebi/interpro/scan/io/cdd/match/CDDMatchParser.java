package uk.ac.ebi.interpro.scan.io.cdd.match;
...
/**
 * Parse a CDD results file.
 *
 * @author Matthew Fraser, EMBL-EBI, InterPro
 * @version $Id$
 * @since 1.0-SNAPSHOT
 */
public class CDDMatchParser extends AbstractLineMatchParser<CDDRawMatch> {
    ...
    public CDDMatchParser(String signatureLibraryRelease) {
        super(SignatureLibrary.PRODOM, signatureLibraryRelease);
    }

    @Override
    protected CDDRawMatch createMatch(String line) {
        line = line.trim();
        ...
        if (!data.find()) {
            ...
            if (!recordEndMatcher.find()) {
                /*
                 * Note that sometimes a result line with no CDD entry is returned. Compare a correct entry:
                ...
                if (LOGGER.isInfoEnabled()) {
                    LOGGER.info("CDD match result in unexpected format ignored: " + line);
                }
            }
            return null;
        }
            ...
            try {
                ...
                switch (i) {
                    ...
                    case 2:
                        ...
                        break;
                    case 4:
                        // CDD model and sp Id, for example: pd_PD000222;sp_SPT1_HUMAN_O43278;
                        if (token.startsWith("pd_") && token.contains(";")) {
                            modelId = token.substring(3, token.indexOf(';'));
                            if (!modelId.startsWith("PD")) {
                                if (LOGGER.isInfoEnabled()) {
                                    LOGGER.info("CDD model: " + modelId + " ignored as it looks invalid. Line: " + line);
                                }
                                return null;
                            }
                        }
                        ...
                        if (modelId == null || spId == null) {
                            if (LOGGER.isInfoEnabled()) {
                                LOGGER.info("CDD model Id/SP Id: " + token + " ignored as it looks invalid. Line: " + line);
                            }
                        }
                        break;
                    case 5:
                    ...
                    case 8:
                        if (token.startsWith("S=")) {
                            ...
                            if (LOGGER.isInfoEnabled()) {
                                LOGGER.info("CDD score: " + token + " ignored as it looks invalid. Line: " + line);
                            }
                            return null;
                        }
                        break;
                    case 9:
                        if (token.startsWith("E=")) {
                            ...
                            if (LOGGER.isInfoEnabled()) {
                                LOGGER.info("CDD evalue: " + token + " ignored as it looks invalid. Line: " + line);
                            }
                            return null;
                        }
                        break;
                    case 11:
                        if (token.startsWith("(") && token.endsWith(")")) {
                            ...
                            if (LOGGER.isInfoEnabled()) {
                                LOGGER.info("CDD number of domains in family: " + token + " ignored as it looks invalid. Line: " + line);
                            }
                            return null;
                        }
                        break;
                ...
                LOGGER.error("Error parsing CDD match output file line (ignoring): " + line + " - Exception " + e.getMessage());
                return null;
            }
        ...
        if (description.contains(key)) {
            ...
            if (keyPos > 0) {
                ...
                if (LOGGER.isInfoEnabled()) {
                    LOGGER.info("CDD description/length text: " + description + " ignored as it looks invalid. Line: " + line);
                }
                return null;
            }
        } else {
        ...
        return new CDDRawMatch(sequenceIdentifier, modelId, this.getSignatureLibraryRelease(),
